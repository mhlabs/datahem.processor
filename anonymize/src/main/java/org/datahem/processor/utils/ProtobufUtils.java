        ...
        if (!f.isRepeated()) {
            ...
            if (forgetField != null) {
                if (fieldType.contains("STRING")) {
                    ...
                    builder.clearField(f);
                    tmp.forEach((val) -> {
                        if (!forgetField.isEmpty())
                            builder.addRepeatedField(f, ByteString.copyFromUtf8(forgetField).toByteArray());
                    });
                } else if (fieldType.contains("INT32")) {
                    List<Object> tmp = (List<Object>) message.getField(f);
                    builder.clearField(f);
                    ...
                    tmp.forEach((val) -> {
                        if (!forgetField.isEmpty())
                            builder.addRepeatedField(f, f.getEnumType().findValueByName(String.valueOf(forgetField)));
                    });
                } else if (fieldType.contains("BOOL")) {
                    List<Object> tmp = (List<Object>) message.getField(f);
                    builder.clearField(f);
