...
import org.datahem.protobuf.measurementprotocol.v1.MPEntityProto.ValEntity;
    ...
    private ValEntity getValues(Parameter p, String val) {
        ...
        try {
            switch (p.getValueType()) {
                case "Integer":
                    valBuilder.setIntValue(Long.parseLong(val));
                    break;
                case "String":
                    valBuilder.setStringValue(val);
                    break;
                case "Boolean":
                    valBuilder.setIntValue(Integer.parseInt(val));
                    break;
                case "Double":
                    valBuilder.setDoubleValue(Double.parseDouble(val));
                    break;
                case "Float":
                    valBuilder.setFloatValue(Float.parseFloat(val));
                    break;
            }
        } catch (Exception e) {
            //LOG.error(e.toString());
